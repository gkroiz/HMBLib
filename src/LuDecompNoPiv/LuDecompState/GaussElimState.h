...
#ifndef MATRIXLIB_GAUSSELIMSTATE_H
#define MATRIXLIB_GAUSSELIMSTATE_H
...
#define PRINT1(msg) { std::unique_lock<std::mutex> lk(mutex1); std::cout << msg << std::flush; }

template <class Type>
class GaussElimState : public AbstractState<MatrixBlockData<Type, 'g'>, MatrixBlockData<Type, 's'>> {
private:
    ...
    size_t bld_ = 0;
public:
    GaussElimState(size_t numBlocksRows, size_t numBlocksCols) {
    ...
    void execute(std::shared_ptr<MatrixBlockData<Type, 's'>> ptr) override {
        ...
        if (getrfCount[ptr->getColIdx()] == 0){
            ...
            this->push(std::make_shared<MatrixBlockData<double, 'g'>>(row, col, blockHeight, blockWidth, ld, ptr->getFullMatrixData(), ptr->getBlockData()));
