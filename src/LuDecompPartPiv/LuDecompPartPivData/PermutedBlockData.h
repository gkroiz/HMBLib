...
#ifndef MATRIXLIB_PERMUTEDBLOCKDATA_H
#define MATRIXLIB_PERMUTEDBLOCKDATA_H


#include "data/MatrixBlockData.h"

template <class Type, char Id = '0', Order Ord = Order::Row>
class PermutedBlockData {
private:
    std::shared_ptr<MatrixBlockData<Type, 's', Ord>> permutedBlock_;

    std::shared_ptr<int> permutationVector_;
public:
    PermutedBlockData(const std::shared_ptr<MatrixBlockData<Type, 's', Ord>> &permutedBlock,
                      const std::shared_ptr<int> &permutationVector) : permutedBlock_(permutedBlock),
...
#endif //MATRIXLIB_PERMUTEDBLOCKDATA_H
